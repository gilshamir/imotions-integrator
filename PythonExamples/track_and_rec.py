    ...
    def connect(self):
        ...
        for notification in notifications:
            ...
            if notification in cbMap:
                ...
                res = self.si.subscribe_to_notificationCB(notification, callback)
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Track and Record feature demo.")
    parser.add_argument(
        "--record-time",
        "-t",
        dest="record_time",
    ...
    parser.add_argument(
        "--ip", dest="ip", default="127.0.0.1", help="RPC server ip address.", type=str
    )
    parser.add_argument(
        "--port", dest="port", default="8100", help="RPC server port.", type=str
    )
    args = parser.parse_args()

    helper = TrackAndRecHelper(args.ip, args.port, args.rec_path, args.log_path)
