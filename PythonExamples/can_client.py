...
class CanClient(object):
    ...
    def __get_appl_config(self):
        ...
        result = self.vxlapi.xlGetApplConfig(
            self.app_name,
            0,
            self.hw_type,
            self.hw_index,
    ...
    def __get_channel_mask(self):
        ...
        self.channel_mask = ctypes.c_ulonglong(
            self.vxlapi.xlGetChannelMask(
                self.hw_type.contents, self.hw_index.contents, self.hw_channel.contents
            )
        )
    ...
    def __open_port(self):
        ...
        self.vxlapi.xlOpenPort.argtypes = [
            ctypes.POINTER(can_types.XLporthandle),
            ctypes.c_char_p,
            can_types.XLACCESS,
            ctypes.POINTER(can_types.XLACCESS),
            ctypes.c_uint,
            ctypes.c_uint,
            ctypes.c_uint,
        ]
    ...
    def __activate_channel(self):
        ...
        result = self.vxlapi.xlActivateChannel(
            self.port_handle, self.channel_mask, XL_BUS_TYPE_CAN, XL_ACTIVATE_NONE
        )
        return result

    def __set_notification(self):
        """Set notification handle"""
        result = self.vxlapi.xlSetNotification(self.port_handle, self.handle, 1)
        return result
    ...
    def __store_can_synch(self, data):
        ...
        if (
            self.start_frame <= self.frame_number <= self.stop_frame
            or self.stop_frame == -1
        ):
            self.json_data[self.frame_number] = {}
            self.json_data[self.frame_number]["TimeStamp"] = time_stamp
            self.json_data[self.frame_number]["EstimatedDelay"] = estimated_delay
    ...
    def __store_can_both_eye_closures(self, data):
        ...
        self.json_data[self.frame_number]["LeftEyelidOpening"] = left_eyelid_opening
        self.json_data[self.frame_number]["LeftEyelidOpeningQ"] = left_eyelid_opening_q
        self.json_data[self.frame_number]["RightEyelidOpening"] = right_eyelid_opening
        self.json_data[self.frame_number]["RightEyelidOpeningQ"] = (
            right_eyelid_opening_q
        )
    ...
    def __store_can_both_pupil_diameters(self, data):
        ...
        self.json_data[self.frame_number]["LeftPupilDiameter"] = left_pupil_diameter
        self.json_data[self.frame_number]["LeftPupilDiameterQ"] = left_pupil_diameter_q
        self.json_data[self.frame_number]["RightPupilDiameter"] = right_pupil_diameter
        self.json_data[self.frame_number]["RightPupilDiameterQ"] = (
            right_pupil_diameter_q
        )

    def __store_can_ascii_key(self, data):
        assci_keyboard_state = self.get_data(1, data, 0)
        self.json_data[self.frame_number]["ASCIIKeyboardState"] = assci_keyboard_state
    ...
    def __store_can_filtered_left_gaze_direction(self, data):
        filtered_left_gaze_direction_x = self.get_data_signed(2, data, 0) / 10000.0
        filtered_left_gaze_direction_y = self.get_data_signed(2, data, 2) / 10000.0
        filtered_left_gaze_direction_z = self.get_data_signed(2, data, 4) / 10000.0
    ...
    def __store_can_filtered_right_gaze_direction(self, data):
        filtered_right_gaze_direction_x = self.get_data_signed(2, data, 0) / 10000.0
        filtered_right_gaze_direction_y = self.get_data_signed(2, data, 2) / 10000.0
        filtered_right_gaze_direction_z = self.get_data_signed(2, data, 4) / 10000.0
    ...
    def __store_can_synch_non_realtime(self, data):
        ...
        if self.start_frame <= self.frame_number:
            ...
            self.json_data[self.frame_number]["EstimatedDelayNonReal"] = estimated_delay

    def __store_can_user_time_stamp_non_real_time(self, data):
        user_time_stamp = self.get_data(8, data, 0)
        self.json_data[self.frame_number]["UserTimeStampNonRealTime"] = user_time_stamp
    ...
    def __store_can_left_blink_closing_amplitude(self, data):
        left_blink_closing_amplitude = self.get_data_signed(2, data, 0) / 10000.0
    ...
    def __store_can_left_blink_opening_amplitude(self, data):
        left_blink_opening_amplitude = self.get_data_signed(2, data, 0) / 10000.0
    ...
    def __store_can_right_blink_closing_amplitude(self, data):
        right_blink_closing_amplitude = self.get_data_signed(2, data, 0) / 10000.0
    ...
    def __store_can_right_blink_opening_amplitude(self, data):
        right_blink_opening_amplitude = self.get_data_signed(2, data, 0) / 10000.0
        ...
        try:
            while self.frame_number < self.stop_frame or self.stop_frame == -1:
                ...
                while not result:
                    ...
                    result = self.vxlapi.xlReceive(
                        self.port_handle, self.msgsrx, self.xl_event
                    )
    ...
    def __write_to_file(self):
        ...
        with open(self.output_file, "w") as log_file:
            log_file.write(json.dumps(self.json_data, indent=2, sort_keys=True))

    def print_json_data(self):
        """Print json data"""
        print(json.dumps(self.json_data))

    ## End of CanClient class
...
def validate_file_path(file):
    ...
    if not os.path.exists(file):
        ...
        raise argparse.ArgumentTypeError("The path {0} does not exist.".format(file))
    return file
...
def print_log(log):
    for frame_number in log:
        ...
        for message in log[frame_number]:
            print("Message: %s Value: %s" % (message, log[frame_number][message]))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Example of how to receive output over CAN."
    )
    parser.add_argument(
        "--vxlapi-path",
        dest="vxlapi_path",
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Start logging from this framenumber. If not specified all frames are logged",
    )
    parser.add_argument(
        "--stop-frame",
        dest="stop_frame",
        type=int,
        default=-1,
        help="Stop logging at this framenumber. If not specified all frames are logged",
    )
    parser.add_argument(
        "--out-dir",
        dest="out_dir",
    ...
    receive_can_data(args.start_frame, args.stop_frame, args.vxlapi_path, args.out_dir)
