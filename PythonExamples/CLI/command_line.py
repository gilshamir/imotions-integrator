...
class ExternalInterfacesCommands(cmd.Cmd):
    ...
    intro = (
        "Welcome to the Smart Eye JSON-RPC ExternalInterface test shell."
    ...
    def do_set_image_source_recording(self, line):
        ...
        if line and len(args) == 1:
            self.ext_interface.set_image_source_recording(args[0])
        elif len(args) == 2:
            self.ext_interface.set_image_source_recording(args[0], args[1])
        else:
            print("Usage: set_image_source_recording sma_path <Optional: smb_path>")
    ...
    def do_set_profile_load_mode(self, line):
        """Set the profile load mode"""
        if line:
            try:
                mode = int(line)
                self.ext_interface.set_profile_load_mode(mode)
            except ValueError:
                print("Invalid profile load mode - Must be an integer value")
        else:
            print("Usage: set_profile_load_mode load_mode")

    def do_get_profile_load_mode(self, line):
        """Set the current profile load mode"""
        self.ext_interface.get_profile_load_mode()

    # World model
    def do_clear_world_model(self, line):
        """Clears the current world model"""
        self.ext_interface.clear_world_model()
        ...
        try:
            if len(args) > 2:
                ...
                try:  # Try to see if IP + PORT was provided
                    self.ext_interface.open_data_stream_udp([args[0], int(args[1])])
                except ValueError:  # Otherwise it is PORT + Log_spec
                    self.ext_interface.open_data_stream_udp(["", int(args[0]), args[1]])
            elif line:
    ...
    def do_close_data_stream_udp(self, line):
        ...
        try:  # Cast to int will cause ValueError when unable to cast
            if len(args) > 1:
                self.ext_interface.close_data_stream_udp([args[0], int(args[1])])
            elif line:
    ...
    def do_open_data_stream_tcp(self, line):
        ...
        try:  # Cast to int will cause ValueError when unable to cast
            if len(args) > 1:
                self.ext_interface.open_data_stream_tcp([int(args[0]), args[1]])
            elif line:
    ...
    def do_start_collect_samples_wcs(self, line):
        ...
        try:  # Cast to int/float will cause ValueError when unable to cast
            if len(args) > 4:
                ...
                print("Usage: start_collect_samples_wcs targetId x y z timeout")
        except ValueError:
    ...
    def do_retrieve_target_samples_statistics(self, line):
        """Orders SEP to respond with data about samples collected for a
        specified target (by id)."""
        args = line.split(" ")
        try:  # Cast to int/float will cause ValueError when unable to cast
            if len(args) == 1:
                self.ext_interface.retrieve_target_samples_statistics(int(args[0]))
            else:
                print("Usage: get_sample_data eye targetId")
        except ValueError:
            print("Invalid input - Must be numerical value")
    ...
    def do_is_gaze_calibrated(self, line):
        """Return bool value of if the gaze is calibrated"""
        self.ext_interface.is_gaze_calibrated()
    ...
    def do_send_notification(self, line):
        ...
        if line:
            ...
            print("Usage: send_notification notificationName optionalParameters")
    ...
    def do_get_camera_image(self, line):
        ...
        try:  # Cast to int/float will cause ValueError when unable to cast
            if len(args) > 1:
                self.ext_interface.get_camera_image([int(args[0]), float(args[1])])
            else:
    ...
    # ProfileId
    def do_clear_single_profile_id(self, line):
        """Clears a single profile id from memory and database"""
        try:  # Cast to int/float will cause ValueError when unable to cast
            if line:
                self.ext_interface.clear_single_profile_id(int(line))
            else:
                print("Usage: clear_single_profile_id profileId")
        except ValueError:
            print("Invalid input - Must be numerical value")

    def do_clear_all_profile_ids(self, line):
        """Clears all profile ids from memory and database"""
        self.ext_interface.clear_all_profile_ids()

    # Modules.
    def do_get_all_modules(self, line):
        """Returns a list of all available modules"""
        self.ext_interface.get_all_modules()

    def do_get_enabled_modules(self, line):
        """Returns a list of currently enabled modules"""
        self.ext_interface.get_enabled_modules()

    def do_enable_module(self, line):
        """Enables a specified module"""
        try:
            if line:
                result = self.ext_interface.enable_module(line)
                print(result)
            else:
                print("Usage: enable_module moduleName")
        except ValueError:
            print("Invalid input - Must be a valid module name")

    def do_disable_module(self, line):
        """Disables a specified module"""
        try:
            if line:
                result = self.ext_interface.disable_module(line)
                print(result)
            else:
                print("Usage: disable_module moduleName")
        except ValueError:
            print("Invalid input - Must be a valid module name")

    def do_get_hardware_info(self, line):
        """Get serial numbers of connected cameras"""
        self.ext_interface.get_hardware_info()
...
def run_command_loop():
    while True:
        try:
            ...
            break
        except external_interfaces.ExternalInterfaceError as e:
            logging.error(f"An error occurred in ExternalInterface: {e}")
