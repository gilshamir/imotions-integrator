...
class SXlLinMsgApi(ctypes.Union):
    _fields_ = [
        ("SXlLinMsg", SXlLinMsg),
        ("SXlLinNoAns", SXlLinNoAns),
        ("SXlLinWakeUp", SXlLinWakeUp),
        ("SXlLinSleep", SXlLinSleep),
...
class SXlTransceiver(ctypes.Structure):
    _fields_ = [("event_reason", ctypes.c_ubyte), ("is_present", ctypes.c_ubyte)]


class SXlTagData(ctypes.Union):
    _fields_ = [
        ("msg", SXlCanMsg),
        ("chipState", SXlChipState),
        ("linMsgApi", SXlLinMsgApi),
        ("syncPulse", SXlSyncPulse),
        ("daioData", SXlDaioData),
        ("transceiver", SXlTransceiver),
    ]
